...
async def test_in_vect_packed(dut):
    ...
    await Timer(1, "ns")
    assert dut.out_vect_packed.value.integer == test_value
...
async def test_in_vect_unpacked(dut):
    ...
    await Timer(1, "ns")
    assert [e.integer for e in dut.out_vect_unpacked.value] == test_value
...
async def test_in_arr(dut):
    ...
    await Timer(1, "ns")
    assert dut.out_arr.value.integer == test_value
...
async def test_in_2d_vect_packed_packed(dut):
    ...
    await Timer(1, "ns")
    assert dut.out_2d_vect_packed_packed.value.integer == test_value
...
async def test_in_2d_vect_packed_unpacked(dut):
    ...
    await Timer(1, "ns")
    assert [e.integer for e in dut.out_2d_vect_packed_unpacked.value] == test_value
...
async def test_in_2d_vect_unpacked_unpacked(dut):
    ...
    await Timer(1, "ns")
    assert [
        [e.integer for e in a] for a in dut.out_2d_vect_unpacked_unpacked.value
    ] == test_value
...
async def test_in_arr_packed(dut):
    ...
    await Timer(1, "ns")
    assert dut.out_arr_packed.value.integer == test_value
...
async def test_in_arr_unpacked(dut):
    ...
    await Timer(1, "ns")
    assert [e.integer for e in dut.out_arr_unpacked.value] == test_value
...
async def test_in_2d_arr(dut):
    ...
    await Timer(1, "ns")
    assert dut.out_2d_arr.value.integer == test_value
...
async def test_in_vect_packed_packed_packed(dut):
    ...
    await Timer(1, "ns")
    assert dut.out_vect_packed_packed_packed.value.integer == test_value
...
async def test_in_vect_packed_packed_unpacked(dut):
    ...
    await Timer(1, "ns")
    assert [e.integer for e in dut.out_vect_packed_packed_unpacked.value] == test_value
...
async def test_in_vect_packed_unpacked_unpacked(dut):
    ...
    await Timer(1, "ns")
    assert [
        [e.integer for e in a] for a in dut.out_vect_packed_unpacked_unpacked.value
    ] == test_value
...
async def test_in_vect_unpacked_unpacked_unpacked(dut):
    ...
    await Timer(1, "ns")
    assert [
        [[e.integer for e in a] for a in b]
        for b in dut.out_vect_unpacked_unpacked_unpacked.value
    ] == test_value
...
async def test_in_arr_packed_packed(dut):
    ...
    await Timer(1, "ns")
    assert dut.out_arr_packed_packed.value.integer == test_value
...
async def test_in_arr_packed_unpacked(dut):
    ...
    await Timer(1, "ns")
    assert [e.integer for e in dut.out_arr_packed_unpacked.value] == test_value
...
async def test_in_arr_unpacked_unpacked(dut):
    ...
    await Timer(1, "ns")
    assert [
        [e.integer for e in a] for a in dut.out_arr_unpacked_unpacked.value
    ] == test_value
...
async def test_in_2d_arr_packed(dut):
    ...
    await Timer(1, "ns")
    assert dut.out_2d_arr_packed.value.integer == test_value
...
async def test_in_2d_arr_unpacked(dut):
    ...
    await Timer(1, "ns")
    assert [[e.integer for e in a] for a in dut.out_2d_arr_unpacked.value] == test_value
...
async def test_in_3d_arr(dut):
    ...
    await Timer(1, "ns")
    assert dut.out_3d_arr.value.integer == test_value
