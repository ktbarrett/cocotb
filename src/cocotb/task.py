...
class Task(Generic[ResultType]):
    ...
    def __init__(self, inst: Coroutine[Trigger, None, ResultType]) -> None:
        ...
        self._coro: Coroutine[Trigger, None, ResultType] = inst
    ...
    def _set_result(
        ...
        for callback in self._done_callbacks:
            _ = cocotb.event_loop._instance.schedule(callback, self)
    ...
    def _resume(self, exc: BaseException | None = None) -> None:
        ...
        if not isinstance(result, Trigger):
    ...
    def kill(self) -> None:
        ...
        result = Value(None)
        self._set_result(result, _TaskState.FINISHED)  # type: ignore  # `kill()` sets the result to None regardless of the ResultType
    ...
    def result(self) -> ResultType:
        ...
        if self._state is _TaskState.CANCELLED:
            ...
            return cast(Outcome, self._result).get()
        else:
