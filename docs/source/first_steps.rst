...
After that we see were the regression module starts ``Running tests``.
...
We have a test running, now we need to to do something useful.
...
The value returned when getting the value depends on the type of the object being accessed.
...
To see the writes take affect, we will need to wait some time.
...
.. note::
    ...
    This prevents any concerns of race conditions or need to lock critical sections.
